...
def nivel_cave():
    ...
    opcao = game_utils.escolhe_opcao("Metes a mão no buraco do lado esquerdo[e], o do lado direito[d] ou subir as escadas[s]?", ["e", "d", "s"])
    if opcao == "e":                
...
def nivel_quarto():
    ...
    opcao = game_utils.escolhe_opcao("Queres abrir a porta[p] ou ligar o interruptor[i]", ["p", "i"])
    if opcao == "p":
        nivel_corredor()   
    else:
...
def jogar():
    ...
    while opcao_sair_ou_continuar != "s":
        ...
        opcao_sair_ou_continuar = game_utils.escolhe_opcao("Queres recomeçar[r] ou sair[s]?", ["r", "s"])